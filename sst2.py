...
def pad_and_redact(data, labels, word_indices):
    ...
    tensor_final_data = [(torch.tensor(data, dtype=torch.float32), torch.tensor(int(label))) for data, label in zip(final_data, labels)]
...
def sst2_data_loader(train_data_filename: str, train_labels_filename: str, dev_data_filename: str, dev_labels_filename: str, test_data_filename: str, feature_type: str, model_type: str):
    """ Loads the data.
    ...
    if model_type == "slp":
        X_train, X_val, X_dev, X_test = sst2_featurize(train_data, val_data, dev_data, test_data, feature_type)
